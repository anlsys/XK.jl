...
\begin{figure*}[]
    \centering
    \includegraphics[width=0.90\linewidth]{figures/xkblas/spmv.pdf}
...
Following our initial motivation, we evaluate XK.BLAS as a backend for Krylov.jl.
...
We use two datasets:
\begin{itemize}
    \item   Three matrices (\texttt{nasarb}, \texttt{pwtk}, \texttt{Queen\_4147}) from the Suite Sparse Matrix Collection (SSMC)~\cite{ssmc}---for realistic test cases. These matrices are all symmetric positive definite, and cover different size ranges (see Table~\ref{tbl:ssmc}). The largest matrix (\texttt{Queen\_4147}) still fits in the memory of a single H100 GPU.
        %
    \item   Three pseudo-random sparse matrix generators ($M_1$, $M_2$, $M_3$) representative of different scientific problems\footnote{Claude Sonnet 4.5~\cite{anthropic-claude-sonnet-45} was used to create the matrix generators.}.
            These matrices allow the exploration of large symmetric positive definite matrix sizes that would not fit on a single GPU.
            They also allow the exploration of different sparsity patterns---that we illustrate on visualizations in Appendix~\ref{sec:appendix-matrices}.
\end{itemize}

% julia> tiny = ssmc[(ssmc.numerical_symmetry .== 1.0) .& (ssmc.positive_definite.== true) .& (ssmc.nnz .>= 100000000), :]
%    3 │ Janna   Queen_4147  4147110  4147110  316548962          4147110                 1.0               true               1.0   false  true  4147110  -2.04835e16+0.0im  4.07611e16+0.0im            47084            47084                47075                47075  70146293583  64402387996  123868718534        12950322        329499284                true                149447   149447  4.87278e15  rsa         false   true
...
%----------------------------%
\subsubsection{Theoretical Complexities} \label{sec:complexities}
%----------------------------%
Table~\ref{tab:complexities} reports the computational and memory complexity of the conjugate gradient solver used throughout this section.
%
...
% Presentation
We evaluate Krylov.jl performance on a single GPU using SSMC matrices.
Table~\ref{tbl:ssmc} compares the performance of XK.BLAS with the default \texttt{cuBLAS} native backend.
...
We evaluate the scaling of the Krylov.jl solver on multiple GPUs using the XK.BLAS backend.
% \paragraph{Protocol}
Table~\ref{tab:multi-gpu} reports the pseudo-random sparse matrices we used to saturate the memory of a single H100 GPU.
...
XK.BLAS not only enables multi-GPU execution of solvers in Krylov.jl but also improves portability: it abstracts available devices by manipulating host-memory objects (\texttt{Vector}, \texttt{Matrix}, \texttt{SparseMatrixCSR}) and automatically moving memory to executing devices.
% AM: Check with Romain if he is using SparseMatrixCSR.jl. Romain: Yes
To implement this backend, we extended XKBlas with support for selected BLAS 1 and 2 routines (\texttt{axpy}, \texttt{dot}, \texttt{scal}, \texttt{gemv}, \texttt{spmv}, etc.).
Evaluations show $2.5\times$ slowdown on the smallest evaluated matrix (\texttt{nasasrb}) due to overhead induced by memory coherence automation.
Overheads are fully amortized on large matrices (e.g., \texttt{Queen\_4147})---and we show up to 2.8× speedup when automatically scaling from 1 to 4 H100 GPUs within the same node.
XK.BLAS also demonstrates portability by executing on three distinct multi-GPU architectures (MI250X, PVC, H100).
...
% Also, please email a copy of the published work to help@jlse.anl.gov.
This research also used resources of the Argonne Leadership Computing Facility at Argonne National Laboratory, which is supported by the Office of Science of the U.S. Department of Energy, Office of Science, under contract number DE-AC02-06CH11357.
...
\emph{Remark:} Sizes differ from our evaluations (Section~\ref{sec:eval}) but the sparsity patterns are identical.
We reduced the sizes to avoid rendering artifacts.
...
\begin{figure}[ht]
    ...
    \begin{minipage}{0.48\columnwidth}
        ~ % Empty space to maintain the 2x2 grid structure
    \end{minipage}
\end{figure}
