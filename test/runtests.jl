...
@testset "XK.jl" begin
    ...
    @testset "BLAS: axpy_async" begin
        ...
        @testset "axpy_async Float64 — partial vector (n < length)" begin
            ...
            @test y[1:n_compute] ≈ expected[1:n_compute] atol=1e-12
        end
        ...
        @testset "axpy_async Float32 — large vector" begin
            ...
            @test y ≈ expected atol=1e-4
        end
    end

    @testset "BLAS: gemm_async" begin
        ...
        @testset "gemm_async Float32 — square identity-like" begin
            ...
            # C should equal A (reshaped as matrix)
            A_mat = reshape(A, m, k)
            C_mat = reshape(C, m, n)
            @test C_mat ≈ A_mat atol=1e-5
        end
    ...
    @testset "memory_coherent_async" begin
        ...
        @testset "vector coherence — Float64 full vector" begin
            # Fill a vector on device, make coherent, check host values
            n = 256
            T = Float64
            fill_val = T(7.0)
        ...
        @testset "vector coherence — Float32 full vector" begin
            ...
            @test all(v -> isapprox(v, fill_val; atol=1e-5), x)
        end
        ...
        @testset "vector coherence — partial (n < length)" begin
            ...
            @test all(v -> v ≈ fill_val, x[1:n_fill])
        end
        ...
        @testset "matrix coherence — Float32 via gemm" begin
            ...
            # Use the top-level matrix coherence wrapper
        ...
        @testset "vector coherence after axpy — verify write-back" begin
            ...
            # Make y coherent and sync
            XK.memory_coherent_async(y)
            XK.sync()

            @test all(v -> v ≈ T(5.0), y)
        end
        ...
        @testset "multiple coherence calls in sequence" begin
            ...
            # Make both coherent before sync
            XK.memory_coherent_async(x)
            XK.memory_coherent_async(y)
            XK.sync()

            @test all(v -> v ≈ T(10.0), x)
            @test all(v -> v ≈ T(20.0), y)
        end
    end

    @testset "BLAS: fill_async" begin
        @testset "fill_async Float32" begin
            ...
            @test all(v -> isapprox(v, val; atol=1e-6), x)
        end
    ...
    @testset "BLAS: scal_async" begin
        @testset "scal_async Float64" begin
            ...
            @test x ≈ alpha .* x_orig atol=1e-10
        end
        ...
        @testset "scal_async Float32" begin
            ...
            @test x ≈ alpha .* x_orig atol=1e-5
        end
    end

    @testset "BLAS: dot_async" begin
        ...
        @testset "dot_async Float32" begin
            ...
            @test result[] ≈ T(n) atol=1e-4
        end
    end

    @testset "BLAS: gemv_async" begin
        ...
        @testset "gemv_async Float32 — with beta" begin
            ...
            @test y ≈ y_ref atol=1e-3
        end
    end

    @testset "BLAS: chained async operations" begin
        @testset "fill -> scal -> axpy -> coherent pipeline" begin
            ...
            # Step 3: axpy y = 2 * x + y = 6 + 2 = 8
            XK.BLAS.axpy_async(n, T(2.0), x, 1, y, 1)

            # Make coherent and sync
            XK.memory_coherent_async(y)
            XK.sync()

            @test all(v -> v ≈ T(8.0), y)
        end
